...
class Button(Sprite):
   ...
   def __init__(self, caption, font, x_pos, y_pos, action, val=None, width=100, height=50, func=None):
    ...
    # UPDATE MOUSE POSITION, RETURNS ACTION OF BUTTON
   ...
   while running:
       ...
       for event in pygame.event.get():
           ...
           if event.type == pygame.KEYDOWN and event.key in [pygame.key.key_code(str(i)) for i in range(1, 10)] and game_board.selected_cell is not None:
               ...
               game_board.place()
               # END MESSAGE WITH ONE BUTTON - RESTART OR EXIT
               if game_board.is_full() is True:  # checking if board is full
                   if game_board.check_board() is False:  # checks if board was filled out correctly
                       # END MESSAGE & BUTTON CREATION IF USER LOST
                       state = STATE.LOST
                       if state == STATE.LOST:
                           font = _font(32)
                           objects = Button("Restart", font, .4 * WIDTH, .70 * HEIGHT, action=STATE.TITLE)
                           running = True
                           while running:
                               font = _font(64)
                               screen.fill('white')
                               screen.blit(font.render("Game Lost :(", False, 'WHITE', 'RED'),
                                           (.25 * WIDTH, .25 * HEIGHT))
                               mouse_up = False
                               for event in pygame.event.get():
                                   if event.type == pygame.QUIT: running = False; break
                                   if event.type == pygame.MOUSEBUTTONUP and event.button: mouse_up = True
                               objects.draw(screen)
                               if objects.update(pygame.mouse.get_pos(), mouse_up) is not None:
                                   return STATE.TITLE
                               pygame.display.flip()
                               clock.tick(60)
                           return STATE.QUIT

                   elif game_board.check_board() is True:  # checks if board was filled out correctly
                       # END MESSAGE & BUTTON CREATION IF USER WON
                       state = STATE.WON
                       if state == STATE.WON:
                           font = _font(32)
                           objects = Button("Exit", font, .4 * WIDTH, .70 * HEIGHT, action=STATE.TITLE)
                           running = True
                           while running:
                               font = _font(64)
                               screen.fill('white')
                               screen.blit(font.render("Game Won!", True, 'WHITE', 'GREEN'),
                                           (.27 * WIDTH, .25 * HEIGHT))
                               mouse_up == False
                               for event in pygame.event.get():
                                   if event.type == pygame.QUIT: running = False; break
